            ...
            <div className="flex flex-row cursor-pointer bg-slate-800 border-slate-700 border duration-200 group-hover:translate-x-2">
                <div className="p-9">
                    <div className="flex flex-row space-x-2 items-center">
                        <svg viewBox="0 0 16 16" className="h-4 w-4 mr-1" fill="#a3a3a3">
                    ...
                    <div className="flex flex-row flex-wrap items-start gap-3 pt-5">
                        {tags.map(element => {
                            return <p key={element} className="text-white text-xs font-medium py-1.5 px-3.5 rounded-full bg-slate-500">{element}</p>
                        })}
                    </div>
                </div>
            </div>
